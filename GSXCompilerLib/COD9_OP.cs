...
{
    internal sealed class COD9_OP : GSCFragment
    {
        private List<byte> OP_Data;
        ...
        internal COD9_OP(List<byte> dlink, byte OP_CODE, bool console) : base(dlink)
        {
            OP_Data = new List<byte>
            {
                0x0
            };
            Code = OP_CODE;
        ...
        {
            Contents = OP_Data.ToArray();
            base.Write();
            /* Fucked right now because the data sources were changed
             * TODO Fix this stupid shit. All ya gotta do is move this code to gscfunction instead. Makes more sense that way anyways. I mean, come on.
        ...
        /*  Note on these two functions that is extremely important:
         *  These will only work if this OP code is written directly after creation
         *  Adding anything before it will throw off alignment and will completely defeat the purpose
         */

        internal void AlignLink16(byte offset = 0)
        {
            var alignedPos = (int)(OP_Data.Count + DataLink.Count + 1 + offset & 0xFFFFFFFE);
            if (OP_Data.Count + DataLink.Count < alignedPos)
                OP_Data.AddRange(new byte[alignedPos - OP_Data.Count + DataLink.Count]);
        }

        internal void AlignLink32(byte offset = 0)
        {
            var alignedPos = (int)(OP_Data.Count + DataLink.Count + 3 + offset & 0xFFFFFFFC);
            if (OP_Data.Count + DataLink.Count < alignedPos)
                OP_Data.AddRange(new byte[alignedPos - OP_Data.Count + DataLink.Count]);
        }
    }
