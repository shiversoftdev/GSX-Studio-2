...
namespace GameScriptCompiler_v3
{
    ...
    /// </summary>
    [Language("GameScript", "Console", "GameScript grammar for the Call of Duty: Black Ops 2")]
    public class GSCGrammar : Grammar
    {
        public GSCGrammar()
        {
            ...
            //Literals
            var numberLiteral = new NumberLiteral("numberLiteral", NumberOptions.AllowSign | NumberOptions.AllowStartEndDot);
            ...
            program.Rule = 
                ...
                includes + usingAnimTree + functions |
                includes + defines + functions |
                includes + functions |
                usingAnimTree + defines + functions |
                usingAnimTree + functions |
                defines + functions |
                functions;
            ...
            declaration.Rule = simpleCall | statement | setVariableField | wait | _return | waitTillFrameEnd | jumpStatement | gotoTerminal | labelTerminal;
            block.Rule = ToTerm("{") + blockContent + "}" | ToTerm("{") + "}";
            blockContent.Rule = declarations;
            parenExpr.Rule = ToTerm("(") + expr + ")";
            expr.Rule = 
                directAccess | call | identifier | protectExpr |
                stringLiteral | array | numberLiteral | 
                vector | expression | relationalExpression | 
                conditionalStatement | boolNot | size |
                        isString | getFunction | hashedString | 
                        getAnimation | animTree | parenExpr;
