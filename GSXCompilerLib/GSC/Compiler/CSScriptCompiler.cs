...
namespace GameScriptCompiler_v3
{
    internal partial class ScriptCompiler
    {
        ...
        private bool STRINGREFTEST = true;
        private bool RAND_ALIGN_TEST = true;
        ...
        //internal static List<GlobalString> GlobalStrings = new List<GlobalString>();
        ...
        {
            CompiledPub.AddRange(BitConverter.GetBytes(i).Reverse<byte>().ToArray<byte>());
        }
        ...
        {
            CompiledPub.AddRange(BitConverter.GetBytes(f).Reverse<byte>().ToArray<byte>());
        }

        private void AddUshort(ushort u)
        {
            CompiledPub.AddRange(BitConverter.GetBytes(u).Reverse<byte>().ToArray<byte>());
        }

        private void AddShort(short s)
        {
            CompiledPub.AddRange(BitConverter.GetBytes(s).Reverse<byte>().ToArray<byte>());
        }
        ...
        internal bool Init()
        {
            ...
            file.PtrToIncludes = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(),0);
            if (includesNodeIndex != -1)
            {
            ...
            file.CodeSectionStart = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            file.NumOfFunctions = BitConverter.ToUInt16(BitConverter.GetBytes((ushort)Tree.Root.ChildNodes[functionsNodeIndex].ChildNodes.Count).Reverse<byte>().ToArray<byte>(), 0);
            ...
            file.GscFunctions = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            file.NumOfFunctions = BitConverter.ToUInt16(BitConverter.GetBytes((ushort)Functions.Count).Reverse<byte>().ToArray<byte>(), 0);
            WriteFunctionsStructToFile();
            file.ExternalFunctions = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            file.NumOfExternalFunctions = BitConverter.ToUInt16(BitConverter.GetBytes((ushort)Calls.Count).Reverse<byte>().ToArray<byte>(), 0);
            WriteCallStackToFile();
            //usinganimtrees struct
            file.PtrTousingAnimTrees = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            WriteAnimTreesToFile();
            file.RefStrings = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            file.NumofRefStrings = BitConverter.ToUInt16(BitConverter.GetBytes((ushort)GetNumRefStrings()).Reverse<byte>().ToArray<byte>(), 0);
            WriteRefsToStringsToFile();
            file.FixUps = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            file.NumFixups = (ushort)WriteFixups();
            file.Size = BitConverter.ToInt32(BitConverter.GetBytes(CompiledPub.Count).Reverse<byte>().ToArray<byte>(), 0);
            if (IsDebug)
            {
                foreach (FixUp f in FixUps)
                {
                    ...
                    CompiledPub.Add(0x0);
                    CompiledPub.AddRange(BitConverter.GetBytes(f.WritePosition).Reverse<byte>().ToArray<byte>());
                    CompiledPub.AddRange(BitConverter.GetBytes(f.FixUpValues.Count).Reverse<byte>().ToArray<byte>());
                }
            }
        ...
        private int WriteFixup(FixUp f)
        {
            ...
            foreach (int ptr in f.FixUpValues.Keys)
            {
                ...
                CompiledPub.AddRange(BitConverter.GetBytes(ptr).Reverse<byte>().ToArray<byte>());
            }
            return total;
        }
        ...
        private void EmitFunction(ParseTreeNode functionNode)
        {
            ...
            CurrentFunction = functionNode.ChildNodes[0].Token.ValueString.ToLower();
            LabelLocations = new Dictionary<string, int>();
            GotoLocations = new Dictionary<int, ParseTreeNode>();
            numofForeachStatements = 0;
            numofParams = 0;
        ...
        private void EmitconditionalStatement(ParseTreeNode node)
        {
            ...
            CompiledPub.Replace(jmpRangePos,
                BitConverter.GetBytes((short)(CompiledPub.Count - jmpRangePos - 2)).Reverse<byte>().ToList());
            ScriptCompile(node.ChildNodes[4]);
            CompiledPub.Replace(opJumpJmpRangePos,
                BitConverter.GetBytes((short)(CompiledPub.Count - opJumpJmpRangePos - 2)).Reverse<byte>().ToList());
        }
        ...
        private void EmitForEachStatement(ParseTreeNode node)
        {
            ...
            foreach (var op in ContinueHistory)
            {
                ...
                BitConverter.GetBytes((short)(CompiledPub.Count - op.JumpRangePos - 2)).Reverse<byte>().ToList());
                    break;
                }
            }
            EvalLocalVariable(arrayKey.second, false);
        ...
        private void EmitSwitchStatement(ParseTreeNode node)
        {
            ...
            CompiledPub.Replace(switchRangePos,
                BitConverter.GetBytes(CompiledPub.Count - switchRangePos - 4).Reverse<byte>().ToList()); //SPOT-1
            AddInt(Switches.Count);
            foreach (Switch Switch in Switches)
            {
                if (Switch.Name is int)
                {
                    CompiledPub.AddRange(new byte[] { 0, 0x80 });
        ...
        private void EmitBreak(bool deleteStartPos = true)
        {
            ...
            foreach (var _break in BreakHistory[BreakHistory.Count - 1])
            {
                CompiledPub.Replace(_break,
                    BitConverter.GetBytes((short)(CompiledPub.Count - _break - 2)).Reverse<byte>().ToList());
            }
        ...
        private void EmitSkipDev(ParseTreeNode Node)
        {
            ...
            CompiledPub.Replace(JmpPos,
                BitConverter.GetBytes((short)(CompiledPub.Count - JmpPos - 2)).Reverse<byte>().ToList());
        }
        ...
        private void EmitBooleanExpr(ParseTreeNode node)
        {
            ...
            if (node.ChildNodes[1].Term.Name == "&&")
            {
                ...
                ScriptCompile(node.ChildNodes[2]);
                CompiledPub.Replace(jmpRangePos,
                    BitConverter.GetBytes((short)(CompiledPub.Count - statementStart)).Reverse<byte>().ToList());
            }
            ...
            {
                CompiledPub.Replace(i, BitConverter.GetBytes((short)(CompiledPub.Count - i - 2)).Reverse<byte>().ToList());
            }
        ...
        private void PatchGotoPointers()
        {
            foreach(int location in GotoLocations.Keys)
            {
                ...
                else
                {
                    ...
                    CompiledPub.Replace(alignedPos, BitConverter.GetBytes(difference).Reverse<byte>().ToList<byte>());
                }
            }
        }
